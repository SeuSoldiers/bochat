...
pub async fn init_schema(pool: &PgPool) -> AppResult<()> {
    ...
    sqlx::query(
        r#"
        CREATE INDEX IF NOT EXISTS idx_group_join_requests_approver_status
        ON group_join_requests(approver_user_id, status, created_at)
        "#,
    )
    .execute(pool)
    .await
    ...
    sqlx::query(
        ...
        ON group_join_requests(requester_user_id, status, created_at)
        "#,
    )
    .execute(pool)
    .await
    ...
    sqlx::query(
        r#"
        CREATE INDEX IF NOT EXISTS idx_audit_logs_bot_id_created_at
        ON audit_logs(bot_id, created_at)
        "#,
    )
    .execute(pool)
    .await
    ...
    sqlx::query(
        r#"
        CREATE INDEX IF NOT EXISTS idx_audit_logs_group_id_created_at
